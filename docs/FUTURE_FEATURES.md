...
- 개인 판매 페이지(`/seller/:memberId`, `/shop/:token`)와 공개 판매자 상품 목록 API 운영 중
- 만료 가능한 판매 페이지 공유 링크 운영 중
- 상품 거래 위치 필드와 Naver Map 검색/지도 선택 운영 중
...
- `ItemSearchCondition` DTO 생성 (keyword, sellerId, status, minPrice, maxPrice)
