...
interface ReservationJpaRepository extends JpaRepository<Reservation, Long> {
	...
	@Query("SELECT r FROM Reservation r " + "JOIN FETCH r.item i " + "JOIN FETCH i.member seller "
			+ "JOIN FETCH r.member buyer " + "WHERE i.id = :itemId " + "AND r.member.id = :reviewerId "
